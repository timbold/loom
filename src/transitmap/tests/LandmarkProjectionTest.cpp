...
void LandmarkProjectionTest::run() {
  {
    Config cfg;
    const char *argv[] = {"prog", "--landmark", "word:ЦАЙЗ,0,0,1,#000,0.5"};
    ConfigReader reader;
  ...
  size_t opPos = svg.find("opacity=\"", pos);
  size_t opEnd = svg.find("\"", opPos + 9);
  double opacity = std::stod(svg.substr(opPos + 9, opEnd - (opPos + 9)));

  // Compute expected coordinates in the SVG after Web Mercator conversion.
  util::geo::DPoint merc = util::geo::latLngToWebMerc(DPoint(0.0001, 0.0001));
  util::geo::Box<double> box = g.getBBox();
  box = util::geo::pad(box,
                       g.getMaxLineNum() * (cfg.lineWidth + cfg.lineSpacing));
